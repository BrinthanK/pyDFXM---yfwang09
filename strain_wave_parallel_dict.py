...
def build_scan_config():
    config = {}
    _propagation_dir = os.path.normpath(
        os.path.join(os.path.dirname(__file__), "..", "propagation")
    )
    ...
    config["psize"] = 40e-9
    config["zl_rms"] = 0.1e-6 / 2.35
    config["zl_truncation_sigma"] = 2.0
    config["Npixels"] = [600, 10, 200]
    config["Nrays"] = 5000000
    config["strain_profile_file"] = os.path.join(
        _propagation_dir, "propagation_info.pkl"
    )
    config["propagation_snap_index"] = 0
    config["phi_start_deg"] = -0.03
    config["phi_stop_deg"] = 0.03
    config["phi_steps"] = 21
    config["num_workers"] = 5
    config["output_dir"] = "data"
    config["output_file"] = "strain_wave_phi_scan_dict_0_small_beam.h5"
    return config
...
def load_strain_profile_dict(filepath, propagation_snap_index=1):

    ext = os.path.splitext(filepath)[1].lower()
    if ext == ".pkl":
        with open(filepath, "rb") as f:
            info = pickle.load(f)
        xs_m = np.asarray(info["X"], dtype=np.float64)
        snaps = info["U_snap"]
        idx = int(propagation_snap_index)
        if idx < 0:
            idx = len(snaps) + idx
        if idx < 0 or idx >= len(snaps):
            raise IndexError(
                "propagation_snap_index %r out of range for %d snapshots"
                % (propagation_snap_index, len(snaps))
            )
        exx = np.asarray(snaps[idx], dtype=np.float64)
        if exx.shape != xs_m.shape:
            raise ValueError(
                "U_snap[%d] shape %s does not match X shape %s"
                % (idx, exx.shape, xs_m.shape)
            )
        return xs_m, exx
    raw = np.load(filepath)
    xs_m = np.asarray(raw["xs_m"], dtype=np.float64)
    exx = np.asarray(raw["exx"], dtype=np.float64)
    return xs_m, exx
...
def writer_worker(queue, writer_config):
    ...
    with h5py.File(saved_h5_file, "w") as hf:
        ...
        hf.attrs["propagation_snap_index"] = config["propagation_snap_index"]
...
def run_parallel_scan(config):
    ...
    snap_idx = config["propagation_snap_index"]
    xs_raw, exx_raw = load_strain_profile_dict(
        config["strain_profile_file"], snap_idx
    )
